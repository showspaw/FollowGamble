                ...
                do {
                    ...
                    if (status == LoginStatus.SUCCESS) {
                        ...
                        if (++errorRetry >= MaxRetry) {
                            executor.execute(()->{
                                log4j2.getInstance().setLog(GenericEnum.Logger.MsgRecorder.name(),"嚴重警告:程式已停止運行, 原因: 資料異常, 請確認法老官方狀態!");
...
//                            dataChangeListener.fireDataChange(Nowrs, this.HeroTableName);
            ...
            //傳送解析後資料至資料管理中心統一控管。
            DataManager.getInstance().add(DataManager.parsePHAKinds(HeroTableName),Nowrs);
//            if(HeroTableName.equals("Soccer"))
//                DataManager.getInstance().add(DataManager.Kinds.PHA_SOCCER,Nowrs);
//            else if(HeroTableName.equals("NBA")){
//                DataManager.getInstance().add(DataManager.Kinds.PHA_NBA,Nowrs);
//            }
        }
        if (isChange()) {
