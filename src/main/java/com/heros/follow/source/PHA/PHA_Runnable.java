...
public class PHA_Runnable implements Runnable {
    ...
    private void completeNotify(String ID) {
        ...
        if (!threadPoolListener.getAlreadyOpen_FollowID().contains(gameResult.getFollowID())) // 不是黑名單 && 還沒開盤過 !DataCenter.getInstence().getIdBanList().contains(gameResult.getFollowID())
        {
...
//				SendApiCenter.getSendApiCenter().sendOpen(SiteCode.PHA.getCode(), threadPoolListener.getHeroTableName(), obj, "");
            }
