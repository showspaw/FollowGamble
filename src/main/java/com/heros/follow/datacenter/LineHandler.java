...
import com.heros.follow.source.datas.*;
...
import static com.heros.follow.utils.GenericEnum.ParserType.HalfStandardLine;
...
public class LineHandler extends Observable implements Observer {
	...
	public void updateSetting() {
		FullStop = DataCenter.getInstence().getC_bean().getFullStop().equals("Y");
		LiveStop = DataCenter.getInstence().getC_bean().getLiveStop().equals("Y");
		autoOpenLive = DataCenter.getInstence().getC_bean().getAutoOpenLive().equals("Y");
		autoOpenLiveTime = Long.parseLong(DataCenter.getInstence().getC_bean().getOpenLiveTime());
	}
	...
	private DataCenter getInstence() {
		return DataCenter.getInstence();
	}
					...
					for (Entry<Integer, LineData> data : mld.getLineMap().entrySet()) {
						...
						try {
							...
							if (!bean.getTypes().get("全部")) {
								switch (GenericMethod.getGameType(mld.getLineType())) {
								case "0":
						...
						try {
							if (GameMap.containsKey(followID)) {
								...
								gameResult.setLeagueName(DataCenter.getInstence().getSingleLeagueDataMap().get(gameData.getLeagueID()).getL_Traditional());
								...
								gameResult.setType(Integer.parseInt(GenericMethod.getGameType(mld.getLineType())));
								refresh = true;
							}
